# Learn React Step by Step

This repository is designed to teach React from the very beginning.
It is written for people who have never learned React before.

You do not need prior React knowledge.
Each concept is explained slowly, clearly, and with simple examples.

---

## What is React?

React is a JavaScript library used to build user interfaces.
In simple terms, React helps you build interactive web pages using reusable pieces called components.

React is commonly used to build:
- Web applications
- Dashboards
- Admin panels
- Interactive user interfaces

---

## How to Use This Repository

This repository is divided into learning steps.
Each step has its own folder and its own README file.

You should follow the steps **in order**.
Do not skip steps, even if they feel simple.

If you are completely new, start from Step 1 below.

---

## Learning Path

This repository is divided into clear learning steps.
Each step has its own folder and a dedicated README file.

Follow the steps **in order**. Do not skip steps.

---

### Step 1: Environment Setup
Learn how to prepare your computer and understand the tools required to work with React.

➡️ Go to: [Step 1 – Setting Up Your Environment](01-setup/README.md)

---

### Step 2: React Basics & First App
Learn what React is and create your first React application.

➡️ Go to: [Step 2 – React Basics & First App](02-react-basics/README.md)

---

### Step 3: Components & JSX
Understand components, JSX, and how React builds UI.

➡️ Go to: [Step 3 – Components & JSX](03-components/README.md)

---

### Step 4: Props
Learn how to pass data into components using props and make components dynamic.

➡️ Go to: [Step 4-props](04-props/README.md)

---

### Step 5: State (useState)
Learn how components manage their own data and update the UI using state.

➡️ Go to: [05-state](05-state/README.md)

---

### Step 6: Events and User Interaction
Learn how React handles user actions such as clicks and input changes.

➡️ Go to: [06-events](06-events/README.md)

---

More steps will be added gradually as you progress.

Each step will have its **own folder, README, and code examples**.
Follow the steps in order to avoid confusion.
